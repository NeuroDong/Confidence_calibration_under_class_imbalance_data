...
def get_equal_bins(probs: List[float], num_bins: int=15) -> Bins:
...
def get_em_bins(probs, labels, num_bins=15):
    sort_index = np.argsort(probs)
    sorted_probs = probs[sort_index]
    sorted_labels = labels[sort_index]
    if num_bins > len(sorted_probs):
        num_bins = len(sorted_probs)
    data = list(zip(sorted_probs, sorted_labels))
    binned_data = split(data, num_bins)
    return binned_data
...
def _get_ce(probs, labels, p, debias, num_bins, binning_scheme, mode='marginal'):
    def ce_1d(probs, labels):
        ...
        if p == 2 and debias:
            ...
            return normal_debiased_ce(get_em_bins(probs,labels), power=p)
...
def debaised_ECE(probs, labels, p=1, debias=True, num_bins = 15, binning_mode = "em", mode='top-label'):
...
class Debaised_ECE(nn.Module):
    def __init__(self,cfg, n_bins=15) -> None:
        super().__init__()
        self.n_bins = n_bins
        self.Need_top_confidence = True
        self.plot_name = "$ECE_{debaised}$"

    def forward(self, probs, labels, p=1, debias=True, num_bins = 15, binning_mode = "em", mode='top-label'):
        return get_binning_ce(probs, labels, p, debias, num_bins = num_bins, binning_mode = binning_mode, mode=mode)