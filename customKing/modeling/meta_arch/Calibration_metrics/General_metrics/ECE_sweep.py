...
class ECE_sweep_em(nn.Module):
  def __init__(self,cfg, n_bins=15) -> None:
    super().__init__()
    self.n_bins = n_bins
    self.Need_top_confidence = True
    self.plot_name = "$ECE_{sweep}$"
    self.Metric_fun = CalibrationMetric(ce_type="em_ece_sweep",num_bins=self.n_bins,bin_method="equal_examples",norm=1)
  def forward(self,confidences,y_list):
    ece_sweep_em = self.Metric_fun.compute_error(confidences,y_list)
    return ece_sweep_em
