...
class MLPBlock(nn.Module):
    ...
    def __init__(self, in_dim: int, mlp_dim: int, dropout: float):
        ...
        nn.init.normal_(self.lin1.bias, std=1e-6)
        nn.init.normal_(self.linear_2.bias, std=1e-6)
    def forward(self,x):
        #x = x.squeeze(dim=0)  #只能用于torchstat
        x = self.lin1(x)
        x = self.act(x)
        x = self.dropout_1(x)
        x = self.linear_2(x)
        x = self.dropout_2(x)
        #x = x.unsqueeze(dim=0)    #只能用于torchstat
        return x
...
class VisionTransformer(nn.Module):
    ...
    def forward(self, batch_images_tensor,label):
        ...
        # else:
        #     batch_images_tensor = self.transforms_evel(batch_images_tensor).cuda().clone().detach()
...
def vit_small(pretrained: bool = False, progress: bool = True, **kwargs: Any) -> VisionTransformer:
    """
...
def Vit_small(cfg):
    return vit_small(image_size=cfg.ImageSize,num_classes=cfg.num_classes,dropout=0.1, attention_dropout=0.1)

@META_ARCH_REGISTRY.register()
def Vit_b_16(cfg):
    return vit_b_16(image_size=cfg.ImageSize,num_classes=cfg.num_classes)

@META_ARCH_REGISTRY.register()
def Vit_b_32(cfg):
    return vit_b_32(image_size=cfg.ImageSize,num_classes=cfg.num_classes)

@META_ARCH_REGISTRY.register()
def Vit_l_16(cfg):
    return vit_l_16(image_size=cfg.ImageSize,num_classes=cfg.num_classes)

@META_ARCH_REGISTRY.register()
def Vit_l_32(cfg):
    return vit_l_32(image_size=cfg.ImageSize,num_classes=cfg.num_classes)