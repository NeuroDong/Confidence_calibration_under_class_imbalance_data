...
class ResNet(nn.Module):
    ...
    def __init__(
        ...
        if len(layers) > 3:
            self.fc = nn.Linear(8*planes * block.expansion, num_classes)
        else:
            self.fc = nn.Linear(4*planes * block.expansion, num_classes)
...
def resnet20(pretrained: bool = False, progress: bool = True, **kwargs: Any) -> ResNet:
    ...
    """
    model = _resnet('resnet20', BasicBlock, [3, 3, 3], pretrained,progress,inplance=16,**kwargs)
    model.conv1 = nn.Conv2d(3, 16, kernel_size=3, stride=1, padding=1,bias=False)
    model.maxpool = nn.Identity()
    model.layer4 = nn.Identity()
    return model

def resnet110(pretrained: bool = False, progress: bool = True, **kwargs: Any) -> ResNet:
    ...
    Args:
        ...
        progress (bool): If True, displays a progress bar of the download to stderr
    """
    model = _resnet('resnet110', BasicBlock, [18, 18, 18], pretrained,progress,inplance=16,**kwargs)
    model.conv1 = nn.Conv2d(3, 16, kernel_size=3, stride=1, padding=1,bias=False)
    model.maxpool = nn.Identity()
    model.layer4 = nn.Identity()
    return model
...
def resnext29_8x64d(pretrained: bool = False, progress: bool = True, **kwargs: Any) -> ResNet:
    ...
    model = _resnet('resnext29_8x4d', Bottleneck, [3, 3, 3],pretrained, progress, **kwargs)
    model.conv1 = nn.Conv2d(3, 64, kernel_size=3, stride=1, padding=1,bias=False)
    model.maxpool = nn.Identity()
    model.layer4 = nn.Identity()
    return model

def resnext29_16x64d(pretrained: bool = False, progress: bool = True, **kwargs: Any) -> ResNet:
    ...
    kwargs['groups'] = 16
    kwargs['width_per_group'] = 64
    model = _resnet('resnext29_32x4d', Bottleneck, [3, 3, 3],pretrained, progress, **kwargs)
    model.conv1 = nn.Conv2d(3, 64, kernel_size=3, stride=1, padding=1,bias=False)
    model.maxpool = nn.Identity()
    model.layer4 = nn.Identity()
    return model
...
def wide_resnet34_2(pretrained: bool = False, progress: bool = True, **kwargs: Any) -> ResNet:
    ...
    return _resnet('wide_resnet34_2', WideBasicBlock, [3, 4, 6, 3],
                   pretrained, progress, **kwargs)
