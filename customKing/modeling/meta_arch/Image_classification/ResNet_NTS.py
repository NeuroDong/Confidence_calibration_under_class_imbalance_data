#参考论文：《Learning to Navigate for Fine-grained Classification》
#该网络的训练模式是多模型一起训练，相互作用！
...
def generate_default_anchor_maps(anchors_setting=None, input_shape=(224,224)):
    """
...
class ProposalNet(nn.Module):
    ...
    def forward(self, x):
        batch_size = x.size(0)
        d1 = self.bn(self.ReLU(self.down1(x)))
        d2 = self.bn(self.ReLU(self.down2(d1)))
        d3 = self.bn(self.ReLU(self.down3(d2)))
...
class attention_net(nn.Module):    #这个是主网络
    def __init__(self,cfg, topN=4):
        super(attention_net, self).__init__()

        class_num = cfg.MODEL.NUM_ClASSES
        self.pretrained_model = resnet50(pretrained=True)
        self.pretrained_model.avgpool = nn.AdaptiveAvgPool2d(1)
        self.pretrained_model.fc = nn.Linear(512 * 4, class_num)
        self.proposal_net = ProposalNet()
        self.topN = topN
        self.concat_net = nn.Linear(2048 * (CAT_NUM + 1), class_num)
        self.partcls_net = nn.Linear(512 * 4,class_num)
        _, edge_anchors, _ = generate_default_anchor_maps(input_shape = cfg.MODEL.INPUT_IMAGESIZE)
        self.pad_side = 224
    ...
    def forward(self, x,label):
        ...
        top_n_cdds = np.stack(top_n_cdds)
        ...
        if self.training:
            #计算损失函数
            part_loss = self.list_loss(part_logits.view(batch * PROPOSAL_NUM, -1),
                                        label.unsqueeze(1).repeat(1, PROPOSAL_NUM).view(-1)).view(batch, PROPOSAL_NUM)
            raw_loss = self.creterion(raw_logits, label)
            concat_loss = self.creterion(concat_logits, label)
            rank_loss = self.ranking_loss(top_n_prob, part_loss)
            partcls_loss = self.creterion(part_logits.view(batch * PROPOSAL_NUM, -1),
                                    label.unsqueeze(1).repeat(1, PROPOSAL_NUM).view(-1))

            total_loss = raw_loss + rank_loss + concat_loss + partcls_loss
            return  concat_logits,total_loss
        else:
    ...
    def ranking_loss(self,score, targets, proposal_num=PROPOSAL_NUM):
...
def resnet50_nts(cfg):
    return attention_net(cfg)
