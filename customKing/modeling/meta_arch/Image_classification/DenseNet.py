...
def densenet_k12_D40(cfg, **kwargs: Any) ->DenseNet:
    model = _densenet('densenet_k12_D40', 12, (12, 12, 12), 16, num_classes = cfg.MODEL.NUM_ClASSES ,reduction=1,**kwargs)
...
def densenet_k12_D100(cfg, **kwargs: Any) ->DenseNet:
    model = _densenet('densenet_k12_D100', 12, (32, 32, 32), 16, num_classes = cfg.MODEL.NUM_ClASSES,reduction=1,**kwargs)
    model.features.conv0 = nn.Conv2d(3, 16, kernel_size=3, stride=1,padding=1, bias=False)
    model.features.pool0 = nn.Identity()
    return model
...
def densenet_k24_D100(cfg, **kwargs: Any) ->DenseNet:
    model = _densenet('densenet_k24_D100', 24, (32, 32, 32), 16, num_classes = cfg.MODEL.NUM_ClASSES,reduction=1,**kwargs)
    model.features.conv0 = nn.Conv2d(3, 16, kernel_size=3, stride=1,padding=1, bias=False)
    model.features.pool0 = nn.Identity()
    return model
...
def densenet_BC_k12_D100(cfg, **kwargs: Any) ->DenseNet:
    N = int((100 - 4)/(3 * 2))
    model = _densenet('densenet_BC_k12_D100', 12, (N, N, N), 12*2, num_classes = cfg.MODEL.NUM_ClASSES,reduction=0.5,**kwargs)
...
def densenet_BC_k24_D250(cfg, **kwargs: Any) ->DenseNet:
    N = int((250 - 4)/(3 * 2))
    model = _densenet('densenet_BC_k24_D250', 24, (N, N, N), 24*2, num_classes = cfg.MODEL.NUM_ClASSES,reduction=0.5,**kwargs)
...
def densenet_BC_k40_D190(cfg, **kwargs: Any) ->DenseNet:
    N = int((190 - 4)/(3 * 2))
    model = _densenet('densenet_BC_k40_D190', 40, (N, N, N), 40*2, num_classes = cfg.MODEL.NUM_ClASSES,reduction=0.5,**kwargs)
...
def densenet121(cfg, **kwargs: Any) -> DenseNet:
    ...
    Args:
        ...
        memory_efficient (bool) - If True, uses checkpointing. Much more memory efficient,
          but slower. Default: *False*. See `"paper" <https://arxiv.org/pdf/1707.06990.pdf>`_.
    """
    return _densenet('densenet121', 32, (6, 12, 24, 16), 64, num_classes = cfg.MODEL.NUM_ClASSES,
                     **kwargs)
...
def densenet161(cfg, **kwargs: Any) -> DenseNet:
    ...
    Args:
        ...
        memory_efficient (bool) - If True, uses checkpointing. Much more memory efficient,
          but slower. Default: *False*. See `"paper" <https://arxiv.org/pdf/1707.06990.pdf>`_.
    """
    return _densenet('densenet161', 48, (6, 12, 36, 24), 96, num_classes = cfg.MODEL.NUM_ClASSES,
                     **kwargs)
...
def densenet169(cfg, **kwargs: Any) -> DenseNet:
    ...
    Args:
        ...
        memory_efficient (bool) - If True, uses checkpointing. Much more memory efficient,
          but slower. Default: *False*. See `"paper" <https://arxiv.org/pdf/1707.06990.pdf>`_.
    """
    return _densenet('densenet169', 32, (6, 12, 32, 32), 64, num_classes = cfg.MODEL.NUM_ClASSES,
                     **kwargs)
...
def densenet201(cfg, **kwargs: Any) -> DenseNet:
    ...
    Args:
        ...
        memory_efficient (bool) - If True, uses checkpointing. Much more memory efficient,
          but slower. Default: *False*. See `"paper" <https://arxiv.org/pdf/1707.06990.pdf>`_.
    """
    return _densenet('densenet201', 32, (6, 12, 48, 32), 64, num_classes = cfg.MODEL.NUM_ClASSES,
                     **kwargs)