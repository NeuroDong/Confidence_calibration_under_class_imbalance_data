...
def _get_SGD_optimizer(Solver,model:torch.nn.Module,index):
    if index == None:
        ...
        return torch.optim.SGD(filter(lambda p : p.requires_grad, model.parameters()),lr=Solver.BASE_LRS[index],momentum=Solver.MOMENTUM,weight_decay=Solver.WEIGHT_DECAY,nesterov=Solver.NESTEROV)

def _get_adam_optimizer(Solver,model:torch.nn.Module,index):
    if index == None:
        return torch.optim.Adam(model.parameters(),lr=Solver.BASE_LR,betas=Solver.BETA,weight_decay=Solver.WEIGHT_DECAY)
    else:
        return torch.optim.Adam(model.parameters(),lr=Solver.BASE_LRS[index],betas=Solver.BETA,weight_decay=Solver.WEIGHT_DECAY)

def build_optimizer(SOLVER: CfgNode,model: torch.nn.Module,index = None):
    """
    Build an optimizer from config.
    """
    if SOLVER.OPTIMIZER == "SGD":
        optimizer = _get_SGD_optimizer(SOLVER,model,index)
    if SOLVER.OPTIMIZER == "Adam":
        optimizer = _get_adam_optimizer(SOLVER,model,index)
    return optimizer
...
# 生成学习率计划表
# ---------------------------------------------------------------------------- #
def build_lr_scheduler(SOLVER: CfgNode, optimizer: torch.optim.Optimizer) -> torch.optim.lr_scheduler._LRScheduler:
    """
    Build an scheduler from config.
    """
    Scheduler = Schedulers(SOLVER,optimizer)
...
class Schedulers():
    ...
    def Step_Decay(self,iteration):
        ...
        position = 0
        for i in range(len(self.Solver.STEPS)):
            if i == 0 and iteration < self.Solver.STEPS[i]:
                break
            elif i != len(self.Solver.STEPS)-1 and iteration >= self.Solver.STEPS[i] and iteration < self.Solver.STEPS[i+1]:
                position = i + 1
                break
            elif i == len(self.Solver.STEPS)-1 and iteration >= self.Solver.STEPS[i]:
                position = i + 1
        
        lr = self.optimizer.defaults["lr"] * (self.Solver.GAMMA ** position)
...
# 生成损失函数
# ---------------------------------------------------------------------------- #
def build_lossFun(cfg,model = None):
    if model == None:
        if cfg.SOLVER.LOSS_FUN == "CrossEntropy":
            return torch.nn.CrossEntropyLoss()
        if cfg.SOLVER.LOSS_FUN == "LeastSquare":
            return LeastSquare_Fun
        if cfg.SOLVER.LOSS_FUN == "KDEloss":
            return KDE_lossFun(cfg)
    else:
        if cfg.SOLVER.LOSS_FUN == "Dirichlet_Fun":
...
class KDE_lossFun(nn.Module):
    def __init__(self,cfg) -> None:
        super().__init__()
        self.class_num = cfg.MODEL.NUM_CLASS
        self.CrossEntropyLoss = torch.nn.CrossEntropyLoss()
